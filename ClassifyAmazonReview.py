...
if __name__ == '__main__':
    ...
    # Decode documents
    decoded = [list(np.exp(autoencoder.get_representation(d))) for d in data]
    data = [list(d) for d in data]
    data_with_decoded = [d + de for d, de in zip(data, decoded)]
    ...
    data_with_decoded_scores = cross_val_score(clf, data_with_decoded, labels, cv=folds)
    print("Data + Representation Vect. Accuracy: %0.2f (+/- %0.2f)" % (
        data_with_decoded_scores.mean(), data_with_decoded_scores.std() * 2))
